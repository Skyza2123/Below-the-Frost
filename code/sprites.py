...
class TransitionSprite(Sprite):
    def __init__(self, pos, size, target, groups):
        # target: identifier or data for the transition destination
        self.target = target
...
class CollideableSprite(Sprite):
    def __init__(self, pos, surf, groups):
        super().__init__(pos, surf, groups)
        # tighten hitbox vertically (exclude top transparent pixels)
        self.hitbox = self.rect.inflate(1, -self.rect.height * 0.5)
        # ensure the collider sits on the sprite's feet/base
